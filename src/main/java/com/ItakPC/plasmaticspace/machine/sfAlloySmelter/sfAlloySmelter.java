...
public class sfAlloySmelter extends BlockContainer{
    ...
    public sfAlloySmelter(boolean state){
        super(Material.iron);
        random = new Random();
        isActive = state;
    }
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        this.blockIcon = iconRegister.registerIcon(Reference.MOD_ID + ":" + (this.isActive ? "alloySmelterSide" : "alloySmelterSide"));
        this.iconTop = iconRegister.registerIcon(Reference.MOD_ID + ":" + (this.isActive ? "alloySmelterTop" : "alloySmelterTop"));
    ...
    public IIcon getIcon(int side, int metadata) {
        return metadata == 0 && side == 3 ? this.iconFront : (side == 1 ? iconTop : (side == metadata ? iconFront : this.blockIcon));
    }
    ...
    public static void updateBlockState(boolean isSmelting, World world, int xCoord, int yCoord, int zCoord) {
        ...
        sfTileAlloySmleter tile = new sfTileAlloySmleter();

        TileEntity entity = world.getTileEntity(xCoord, yCoord, zCoord);
        keepInventory = true;

        if(tile.isSmelting()) {
            world.setBlock(xCoord, yCoord, zCoord, PlasmaticSpace.sfAlloySmelterActive);
        }else{
            world.setBlock(xCoord, yCoord, zCoord, PlasmaticSpace.sfAlloySmelterIdle);
        }
